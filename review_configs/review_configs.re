...
三次元点が3点あり、それが正規直交基底である場合、$R = R@<b>{2 R}1^{-1}`$で求めることができる。
...
scipyには、@<tt>{align_vectors}というメソッドがあり、このメソッドを使えば、ベクトルのリストを2つ用意するのみで、回転を求められる。
...
@<href>{https://www.tensorflow.org/lite/guide#next_steps,TFLiteはEdgeデバイスの上でモデルを動かすことを想定して作られています。}
 ...
 * バイナリサイズが小さい
 * @<href>{https://www.tensorflow.org/lite/performance/post_training_quantization#optimization_methods,量子化}



デバイスで高速に実行するために、演算が一部の用途に特化した実装になっていたり、
@<href>{https://www.tensorflow.org/lite/guide/ops_compatibility,サポートされている演算に制約}があったりするようです。
...
TensorRTは対応しているが、
@<href>{https://github.com/google/mediapipe/issues/723,TensorRTを使うにはビルド時のフラグを変える必要}が有るとのことです。
...
これは、@<href>{https://google.github.io/styleguide/cppguide.html#Exceptions,Googleの既存プロダクトで例外があまり使われていないという特殊な事情}が関係していて、Googleが公開しているOSSもスタイルガイドの影響を受けているからこうなっています。



基本的にmediapipeの失敗する可能性のあるメソッドの多くは@<tt>{Status}クラスを返却するようになっていて、
値を返却するメソッドに関しては、@<tt>{StatusOr<T>}を返却するようになっています。
 ...
 * Orthogonal Projection: XY平面への投影＝単にZ軸を取り除くのみ。@<href>{https://en.wikipedia.org/wiki/Orthographic_projection,変換行列}を見るとわかりやすい。
 ** CGやコンピュータビジョンで使っているのは見たことがない
 * @<href>{https://en.wikipedia.org/wiki/3D_projection,Weak Perspective Projection: }
 ...
 ** @<href>{https://docs.unity3d.com/ja/2018.4/Manual/PhysicalCameras.html,Unityなど3Dエンジンの描画に使われます。}
 * 更に複雑なモデル
...
例えば、@<href>{https://github.com/google/mediapipe/issues/742,これ}。
...
@<href>{https://github.com/google/mediapipe/releases/tag/v0.7.6,v0.7.6}でZをnormalizeするオプションがついていますが、このnormalizeでは、おそらく、z軸の値の範囲を、
...
また、mediapipeのhand trackingは推論を行うデバイスで@<href>{https://github.com/google/mediapipe/issues/99#issuecomment-531301021,カメラキャリブレーションなどなしで動くアルゴリズムを提供したいらしく}、グローバルなZ座標推定にカメラキャリブレーションが必要な現状では、3次元座標推定を提供しないとのことです。
...
例えば、2D->3D Liftingでは、@<href>{https://research.fb.com/wp-content/uploads/2019/05/3D-human-pose-estimation-in-video-with-temporal-convolutions-and-semi-supervised-training.pdf,カメラの内部パラメータが入力として必要です。}



嘆願ベースのデプス推定でも、@<href>{https://arxiv.org/pdf/2003.06620.pdf,intrinsic parameterは必要な手法が大半}のようだが@<href>{https://arxiv.org/pdf/1904.04998.pdf,、カメラの内部パラメータを推定するネットワークを組み込んで}、不要なものもあるようです。@<href>{https://github.com/google-research/google-research/tree/master/depth_from_video_in_the_wild,camera intrinsic不要なモデルは公式コードもあり。}
...
== 要件の分類
 * hard deadline or soft deadline認識パイプラインはほとんどsoft deadline?アプリ側でカバーできるはず。
